(ns fundmgmt.operation
  "OperationActor -- one fund-management operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (FundManager-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the FundManagementGovernor (:govern) and the
  rollout phase gate (:decide) before anything commits to the SSoT.

  One graph run = one fund-management operation (intake -> advise ->
  ...
  decision to a human GP principal. `:fee/drawdown` ALWAYS reaches this
  node when the governor is clean -- see `fundmgmt.phase`."
...
(defn build
  ...
  `fundmgmt.store/Store`).
  opts:
    :advisor      -- a `fundmgmt.advisor/Advisor` (default: mock-advisor)
